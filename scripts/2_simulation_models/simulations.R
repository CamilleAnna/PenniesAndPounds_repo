#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
#                         Pound&Pennies paper                          #
#       Simulation of the different interventions & save output        #
#                        Last edit 24/07/2021                          #
#                          Camille Simonet                             #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #

# NOTE:
# in script, "inapropriate_usage" is bystander selection
...
# to launch R script from shell script for parallel run with external options defined in shell script
option_list = list(
...
source('2_scripts/2_simulations/models.R') # script with models functions to feed in deSolve
...
# run the burn-in phase (300 time points)
# check presence of resistance (& prevalence of disease not zero) at the end of burn-in phase, if below 10% minimum treshold, draw a new parameter combination
# store end of burn-in phase state for initial state
# run the different models using that parameter combination and this initial state
# record variables for downstream analyses
# store simulation RData output object
...
# each draw contains all parameters, but not all of them is necessarily used by all models, only the relevant ones for the given model
# so all models are run on same sample of parameters +/- the ones specific to each model
# Some filtering apply: 
# draws are not rounded so never equal to zero
# bb < bw, because we set bb = [0-1] * bw
# must lead to a minimum level of resistance (10%) at the end of burn in phase
# then for paper figure plots, we filtered simulations to keep only releavant ones, that is, those where front line drug is used more
...
# initial states, only front line treated individuals,
# all resistance profiles possible present, but majority of sensitive
state_burn_in = c(C_00_u = 0, C_00_f = 0, I_00_f = 0.5, I_00_l = 0, 
                  C_10_u = 0, C_10_f = 0, I_10_f = 0.5/57, I_10_l = 0,  
                  C_01_u = 0, C_01_f = 0, I_01_f = 0.5/57, I_01_l = 0, 
                  C_11_u = 0, C_11_f = 0, I_11_f = 0.5/57, I_11_l = 0,
                  D = 0, dTrans = 0) # D and dTrans record the number of death and number of transmission. Used to compute measures of public health outcome
...
# To do adjustment of initial conditions where necessary (see later)
# DIAGNOSTIC MODEL
...
records<- seq(0, tail(times,1), 1)  # thin records


# To store simulations when several are launched in a single run
...
# filtering criteria to validate a parameter combination draw
min_freq_res<- 0.1 # minimum resistance frequency (10%)
min_numeric<- 0.1  # minimum prevalence of disease
index = 0   # to record draw number, index = index + 1 every time we have to re-draw parameters for a given simulation
...
while(i < nb_sim + 1){
  ...
  reached_min_resistance<- FALSE
  reached_min_numeric<- FALSE
  
  # re-draw new combinations until filtering criteria are validated
  while(reached_min_resistance == FALSE | reached_min_numeric == FALSE){
    ...
    outs_burn_in[[i]]<- outs_burn_in[[i]][outs_burn_in[[i]][,'time'] %in% seq(0, tail(burn_in_time,1)),] # thin the records
    ...
    initial_states[[i]]<- tail(outs_burn_in[[i]], 1)[,-1] # record initial states to use for intervention deployment
...
# in the model, we do not have anymore front line resistants being front line treated in the symptomatic class, i.e. there is no I_10_f or I_11_f
...
# Within each element of the list, there is one entry per model (no_int, adj_f, adj_l, diag_f, diag_l, new_f, new_l)
  ...
  runs[[i]]$no_int<- runs[[i]]$no_int[runs[[i]]$no_int[,'time'] %in% records,] # thinning records
  
  
  
  # ADJUVANT: mod2 and mod3, ini = normal initial_states
  runs[[i]]$adj_f<-  ode(y = initial_states[[i]], times = times, func = mod2, parms = pars[i,])
  runs[[i]]$adj_f<- runs[[i]]$adj_f[runs[[i]]$adj_f[,'time'] %in% records,] # thinning records
  
  
  runs[[i]]$adj_l<-  ode(y = initial_states[[i]], times = times, func = mod3, parms = pars[i,])
  runs[[i]]$adj_l<- runs[[i]]$adj_l[runs[[i]]$adj_l[,'time'] %in% records,] # thinning records
  ...
  # store vectors of character strings to extract the right variables of the model output to compute the integral over for the different measures
 ...
 # c(a = 2,
  ...
  # get the public health measures by computing integral over the appropriate quantities, on the appropriate model output
  ...
  out_data[i,]<- c(
    ...
    # check if there are negative values in the burnin phase, or in any of the interventions run
  ...
  # Those get overwritten at each loop if several simulations are to be run
  # RData object is a list with the different simulation output
  # out_data is a tab delimited table with as many rows as there are simulations
